...
interface IMasterChefCronus {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this poolInfo. CRN to distribute per block.
        uint256 lastRewardTimestamp; // Last block timestamp that CRN distribution occurs.
        uint256 accCrnPerShare; // Accumulated CRN per share, times 1e12. See below.
        IRewarder rewarder;
    }
...
/**
 * This is a sample contract to be used in the MasterChefCronus contract for partners to reward
 * stakers with their native token alongside CRN.
 *
 * It assumes no minting rights, so requires a set amount of YOUR_TOKEN to be transferred to this contract prior.
 * E.g. say you've allocated 100,000 XYZ to the CRN-XYZ farm over 30 days. Then you would need to transfer
...
contract SimpleRewarderPerSec is IRewarder, BoringOwnable, ReentrancyGuard {
    ...
    /// @notice Info of each MCC poolInfo.
    ...
    IMasterChefCronus public immutable MCC;
    ...
    modifier onlyMCC() {
        require(msg.sender == address(MCC), "onlyMCC: only MasterChefCronus can call this function");
        _;
    }
    ...
    constructor(
        ...
        IMasterChefCronus _MCC,
        bool _isNative
    ) public {
        require(Address.isContract(address(_rewardToken)), "constructor: reward token must be a valid contract");
        require(Address.isContract(address(_lpToken)), "constructor: LP token must be a valid contract");
        require(Address.isContract(address(_MCC)), "constructor: MasterChefCronus must be a valid contract");
    ...
    /// @notice Function called by MasterChefCronus whenever staker claims CRN harvest. Allows staker to also receive a 2nd reward token.
    /// @param _user Address of user
    /// @param _lpAmount Number of LP tokens the user has
    function onCronusReward(address _user, uint256 _lpAmount) external override onlyMCC nonReentrant {
        updatePool();
    ...
    function pendingTokens(address _user) external view override returns (uint256 pending) {
        ...
        uint256 lpSupply = lpToken.balanceOf(address(MCC));
